...
function load () {
  ...
  updateBtn.addEventListener('click', function () {
    ...
    if (networks[currentNetWork] !== undefined) {
      ...
      container.innerHTML = 'current network not available through bgmchainscan API'
    }
  })
...
function getBlockNumber (callback) {
  getToken(function (error, apikey) {
    if (error) console.log(error)
    httpGetAsync('https://api-' + networks[currentNetWork] + '.bgmchainscan.io/api?module=proxy&action=bgm_blockNumber&apikey=' + apikey, function (result) {
...
function getLatestBlockInfo (callback) {
  getToken(function (error, apikey) {
    if (error) console.log(error)
    httpGetAsync('https://api-' + networks[currentNetWork] + '.bgmchainscan.io/api?module=proxy&action=bgm_getBlockByNumber&tag=latest&boolean=true&apikey=' + apikey, function (result) {
...
function getGasPrice (callback) {
  getToken(function (error, apikey) {
    if (error) console.log(error)
    httpGetAsync('https://api-' + networks[currentNetWork] + '.bgmchainscan.io/api?module=proxy&action=bgm_gasPrice&apikey=' + apikey, function (result) {
