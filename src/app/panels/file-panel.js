...
function filepanel (localRegistry) {
    ...
    packageFiles(self._deps.fileProviders[fileProviderName], (error, packaged) => {
      if (error) {
        ...
        if (!tokenAccess) {
          ...
          var description = 'Created using remix-ide: Realtime Bgmchain Contract Compiler and Runtime. \n Load this file by pasting this gists URL or ID at https://remix.bgmchain.org/#version=' + queryParams.get().version + '&optimize=' + queryParams.get().optimize + '&gist='
          var gists = new Gists({
            token: tokenAccess
          })
          if (id) {
  ...
  function copyFiles () {
    modalDialogCustom.prompt(null, 'To which other remix-ide instance do you want to copy over all files?', 'https://remix.bgmchain.org', (target) => {
      doCopy(target)
    })
