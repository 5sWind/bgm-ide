...
var Personal = require('web3-bgm-personal')
  ...
  self._components.transactionContextAPI = {
    ...
    getValue: (cb) => {
      try {
        ...
        var unit = 'bgmchain' // default
        if (selectedUnit === 'bgmchain') {
          unit = 'bgmchain'
...
function updateAccountBalances (container, self) {
  ...
  accounts.each(function (index, value) {
    (function (acc) {
      self._deps.udapp.getBalanceInBgmchain(accounts[acc].value, function (err, res) {
        if (!err) {
...
function contractDropdown (events, self) {
  ...
  self._deps.pluginManager.event.register('sendCompilationResult', (file, source, languageVersion, data) => {
    // TODO check whbgmchain the tab is configured
  ...
  function createInstance (args, compiler) {
    ...
    if (selectedContract.contract.object.evm.deployedBytecode && selectedContract.contract.object.evm.deployedBytecode.object.length / 2 > 24576) {
      ...
      More info: <a href="https://github.com/bgmchain/EIPs/blob/master/EIPS/eip-170.md" target="_blank">eip-170</a>
      </div>`,
        {
          label: 'Force Send',
          fn: () => {
  ...
  function loadFromAddress () {
    ...
    if (!bgmJSUtil.isValidAddress(address)) {
      return modalDialogCustom.alert('Invalid address.')
    }
    if (/[a-f]/.test(address) && /[A-F]/.test(address) && !bgmJSUtil.isValidChecksumAddress(address)) {
  ...
  var environmentEl = yo`
    <div class="${css.crow}">
      ...
      <div class=${css.environment}>
        ...
        <a href="https://github.com/bgmchain/EIPs/blob/master/EIPS/eip-155.md" target="_blank"><i class="${css.icon} fa fa-info"></i></a>
      </div>
    </div>
  `
  var accountEl = yo`
  ...
  var valueEl = yo`
    <div class="${css.crow}">
      ...
      <select name="unit" class="${css.col2_2}" id="unit">
        ...
        <option data-unit="bgmchain">bgmchain</option>
      </select>
    </div>
  `
  // DOM ELEMENT
    ...
    self._deps.udapp.getAccounts((err, accounts) => {
      ...
      if (isVM) {
        modalDialogCustom.promptMulti(signMessageDialog, (message) => {
          const personalMsg = bgmJSUtil.hashPersonalMessage(Buffer.from(message))
          var privKey = self._deps.udapp.accounts[account].privateKey
          try {
            var rsv = bgmJSUtil.ecsign(personalMsg, privKey)
            var signedData = bgmJSUtil.toRpcSig(rsv.v, rsv.r, rsv.s)
        ...
        modalDialogCustom.promptMulti(signMessageDialog, (message) => {
          ...
          try {
            executionContext.web3().bgm.sign(account, hashedMsg, (error, signedData) => {
