...
UniversalDApp.prototype.createVMAccount = function (privateKey, balance, cb) {
  ...
  cb(null, '0x' + bgmJSUtil.privateToAddress(privateKey).toString('hex'))
}
...
UniversalDApp.prototype.newAccount = function (password, cb) {
  if (!executionContext.isVM()) {
    ...
    } while (!bgmJSUtil.isValidPrivate(privateKey))
...
UniversalDApp.prototype._addAccount = function (privateKey, balance) {
  ...
  if (self.accounts) {
    privateKey = Buffer.from(privateKey, 'hex')
    var address = bgmJSUtil.privateToAddress(privateKey)
...
UniversalDApp.prototype.getAccounts = function (cb) {
  ...
  if (!executionContext.isVM()) {
    ...
    // See: https://github.com/bgmchain/web3.js/issues/442
    if (this._deps.config.get('settings/personal-mode')) {
      executionContext.web3().personal.getListAccounts(cb)
    } else {
      executionContext.web3().bgm.getAccounts(cb)
    }
  } else {
    if (!self.accounts) {
...
UniversalDApp.prototype.getBalance = function (address, cb) {
  ...
  address = bgmJSUtil.stripHexPrefix(address)

  if (!executionContext.isVM()) {
    executionContext.web3().bgm.getBalance(address, function (err, res) {
      if (err) {
        cb(err)
      } else {
...
UniversalDApp.prototype.getBalanceInBgmchain = function (address, callback) {
  var self = this
  self.getBalance(address, (error, balance) => {
    if (error) {
      callback(error)
    } else {
      callback(null, executionContext.web3().fromWei(balance, 'bgmchain'))
    }
  })
}
    ...
    if (!error) {
      ...
      self.callFunction(args.address, data, args.funABI, (error, txResult) => {
        if (!error) {
          ...
          if (lookupOnly) {
            var decoded = uiUtil.decodeResponseToTreeView(executionContext.isVM() ? txResult.result.vm.return : bgmJSUtil.toBuffer(txResult.result), args.funABI)
            outputCb(decoded)
          }
        } else {
    ...
    function runTransaction (fromAddress, value, gasLimit, next) {
      ...
      self.txRunner.rawRun(tx,
        ...
        (network, tx, gasEstimation, continueTxExecution, cancelCb) => {
          ...
          var amount = executionContext.web3().fromWei(typeConversion.toInt(tx.value), 'bgmchain')
          var content = confirmDialog(tx, amount, gasEstimation, self,
            (gasPrice, cb) => {
              ...
              try {
                ...
                txFeeText = ' ' + executionContext.web3().fromWei(fee.toString(10), 'bgmchain') + ' Bgmchain'
                priceStatus = true
              } catch (e) {
            ...
            (cb) => {
              executionContext.web3().bgm.getGasPrice((error, gasPrice) => {
